...
def maintain_varible1(workstations, robots):
    ...
    material_locked_list = get_material_locked1(workstations)
    for robot_id in range(len(robots)):
...
def distance_buy_sell1(workstations, station_id):
...
def check_action1(workstations, robots):
...
def find_target1(r_distance, r_priority, sell_priority, workstations, robots):
    alphamai = 0
    alpha = 0
    # alpha = 50
    for robot_id in range(len(robots)):
        if r_order[robot_id] == 0:  # 如果当前机器人没有靶订单，则设置靶订单
            ...
            if robots[robot_id]['if_product'] == 0:
                ...
                for i in range(len(r_priority)):
                    prior = r_priority[i]
                    if prior > max_priority and r_next_pass1(r_next, robots, workstations,
                                                            robot_id, i, flag=0):
                        max_priority = prior
                        r_next[robot_id] = i
                        r_order[robot_id] = 1
                ...
                if product_id == 1:
                    ...
                    for i in range(len(sell_priority_copy)):
                        ...
                        if prior > max_priority and product_id in find_materials_id(
                                workstations[i]['type'], workstations[i]['m_state']) and r_next_pass1(r_next, robots,
                                                                                                     workstations,
                                                                                                     robot_id, i,
                                                                                                     flag=1):
                            max_priority = prior
...
def compute_real_direction1(delta_direction, delta_y):
    # 区分象限，以机器人的坐标为 (0,0)
    if delta_y > 0:
...
def move_target1(r_distance, workstations, robots):
    global locked_robots
    global r_next
    global r_action
    global r_order
    ...
    for robot_id in range(len(robots)):
        ...
        real_delta_direction = compute_real_direction1(delta_direction, delta_y)
        ...
        for robot_j in range(len(robots)):
            if robot_j > robot_id:
                if if_crash(robots, robot_id, robot_j):
                    if frame_id % 2 == 0:
                        evade_crash1(robots, robot_id, robot_j)
                    elif frame_id % 2 == 1:
                        evade_crash_simple1(robots, robot_id, robot_j)
                    # elif frame_id % 3 == 2:
                    #     evade_crash_back(robots, robot_id, robot_j)

def evade_crash_back1(robots, robot_i, robot_j):
    r_action[robot_j][0] = -2
    r_action[robot_j][1] = - np.sign(r_action[robot_j][1]) * np.pi


def evade_crash_simple1(robots, robot_i, robot_j):
    second = 0.1
...
def evade_crash1(robots, robot_i, robot_j):
    agents = [-1, -1, -1, -1]
    max_speed = 6
    dt = 1/50
    tau = 5
    ...
    for i, agent in enumerate(agents):
        ...
        if new_speed != []: 
            ...
            real_delta_direction = compute_real_direction1(delta_direction, new_speed[1])
...
def get_material_locked1(workstations):
...
def r_next_pass1(r_next, robots, workstations, robot_i, station_i, flag=0):
...
def handle_module1(workstations, robots, frame_id, money):
    global r_action
    global locked_robots
    r_action = [[-1] * 5, [-1] * 5, [-1] * 5, [-1] * 5]
    locked_robots = []
    # Maintain the distance for each robot and s_type for stations.
    r_distance, r_priority, sell_priority = maintain_varible1(workstations, robots)
    
    # Update r_next for each robot.
    find_target1(r_distance, r_priority, sell_priority, workstations, robots)
    
    move_target1(r_distance, workstations, robots)
    
    # Check and update buy, sell, destroy action for each robot.
    check_action1(workstations, robots)
...
def maintain_varible2(workstations, robots):
    # Compute distance between robot and station
    for robot_id in range(len(robots)):
        robot_i = []
        for station_id in range(len(workstations)):
    ...
    global locked_robots
    # 获取每一帧，物品 1-7, 哪个类别物品锁住了。
    material_locked_list = get_material_locked2(workstations)
    for robot_id in range(len(robots)):
        product = robots[robot_id]["if_product"]
        if product != 0 and product in material_locked_list:
            # 机器人有材料，且没地方放
    ...
    for station_id in range(len(workstations)):
        ...
        if workstations[station_id]['type'] in [1]:
            priority = 5
            
        if workstations[station_id]['type'] in [3]:
            priority = 20
    ...
    for station_id in range(len(workstations)):
        ...
        if workstations[station_id]['type'] in [4, 5]:
            priority = 20
        
        if workstations[station_id]['type'] in [6]:
            priority = 25

        if workstations[station_id]['type'] in [7] and frame_id < 7000:
            priority = 30
        elif workstations[station_id]['type'] in [7] and frame_id > 7000:
            priority = 40

        # 当前工作站空缺的材料位
        material_num = len(find_materials_id(workstations[station_id]['type'], workstations[station_id]['m_state']))
        # 补3缺2
        if workstations[station_id]['type'] in [7] and material_num == 2 and frame_id < 7000:
            priority += 5
        elif workstations[station_id]['type'] in [7] and material_num == 2 and frame_id > 7000:
            priority += 10
        # 补3缺1
        if workstations[station_id]['type'] in [7] and material_num == 1 and frame_id < 7000:
            priority += 20
        elif workstations[station_id]['type'] in [7] and material_num == 2 and frame_id > 7000:
            priority += 25

        # 补2缺1
        if workstations[station_id]['type'] in [4, 5] and material_num == 1:
            priority += 10
        # 补2缺1
        if workstations[station_id]['type'] in [6] and material_num == 1:
            priority += 15
...
def distance_buy_sell2(workstations, station_id):
    # 输入买的station id，判断到其最近可卖的点的距离
    # 买的 station id 的类别是 1-7
    current_type = workstations[station_id]["type"]
    buy_sell_list = [[], [4,5,9], [4,6,9], [5,6,9], [7,9], [7,9], [7,9], [8,9]]
...
def check_action2(workstations, robots):
    # Check for buy, sell, destroy for each robot.
    for robot_id in range(len(robots)):
        target_station = r_next[robot_id]
        robot_in_station = robots[robot_id]["if_station"]
...
def find_target2(r_distance, r_priority, sell_priority, workstations, robots):
    alphamai = 1.5
    alpha = 1.5
    # alpha = 50
    r_s_dis = r_s_distance(robots, workstations)
    r_s_dis_mai = r_s_distance_mai(robots, workstations)
    for robot_id in range(len(robots)):
        if r_order[robot_id] == 0:  # 如果当前机器人没有靶订单，则设置靶订单
            r_priority = np.array(r_priority)
            sell_priority = np.array(sell_priority)
            if robots[robot_id]['if_product'] == 0:
                ...
                for i in range(len(r_priority)):
                    buy2sell_distance = 0
                    if workstations[i]["type"] in [1,2,3,4,5,6]:
                        buy2sell_distance = distance_buy_sell2(workstations, i)
                    prior = r_priority[i] - alphamai * r_s_dis_mai[robot_id][i] - buy2sell_distance
                    if prior > max_priority and r_next_pass2(r_next, robots, workstations,
                                                            robot_id, i, flag=0):
                        max_priority = prior
                        r_next[robot_id] = i
                        r_order[robot_id] = 1
                ...
                if product_id == 1:
                    ...
                    max_priority = -np.inf
                    for i in range(len(sell_priority_copy)):
                        prior = sell_priority_copy[i] - alpha * r_s_dis_mai[robot_id][i]
                        if prior > max_priority and product_id in find_materials_id(
                                workstations[i]['type'], workstations[i]['m_state']) and r_next_pass2(r_next, robots,
                                                                                                     workstations,
                                                                                                     robot_id, i,
                                                                                                     flag=1):
                            max_priority = prior
...
def compute_real_direction2(delta_direction, delta_y):
    # 区分象限，以机器人的坐标为 (0,0)
    if delta_y > 0:
        if delta_direction > 0 and delta_direction < np.pi:
            real_delta_direction = - delta_direction
...
def move_target2(r_distance, workstations, robots):
    global locked_robots
    global r_next
    global r_action
    global r_order
    ...
    for robot_id in range(len(robots)):
        ...
        delta_direction = robot_direction - direction
        real_delta_direction = compute_real_direction2(delta_direction, delta_y)
        ...
        eps = 1.5
        theta = 0.1
        if 1 in dis_wall2(robot_id, eps):
            if robot_direction > (np.pi / 2) + theta and robot_direction < 3 * np.pi / 2 - theta:
                if abs(real_delta_direction) > 0.7:
                    ...
                    r_action[robot_id][1] = np.pi * (ex - ex2) / (ex + ex2)
        if 3 in dis_wall2(robot_id, eps):
            if robot_direction > np.pi + theta and robot_direction < 2 * np.pi - theta:
                if abs(real_delta_direction) > 0.7:
                    ...
                    r_action[robot_id][0] = 2
                    r_action[robot_id][1] = np.pi * (ex - ex2) / (ex + ex2)
        if 2 in dis_wall2(robot_id, eps):
            if (robot_direction >= 0 and robot_direction < np.pi / 2 - theta) or (
                ...
                if abs(real_delta_direction) > 0.7:
                    ...
                    r_action[robot_id][0] = 2
                    r_action[robot_id][1] = np.pi * (ex - ex2) / (ex + ex2)
        if 4 in dis_wall2(robot_id, eps):
    ...
    for robot_id in range(len(robots)):
        for robot_j in range(len(robots)):
            if robot_j > robot_id:
                if if_crash(robots, robot_id, robot_j):
                    if frame_id % 3 == 0:
                        evade_crash2(robots, robot_id, robot_j)
                    elif frame_id % 3 == 1:
                        evade_crash_simple2(robots, robot_id, robot_j)
                    # elif frame_id % 3 == 2:
                    #     evade_crash_back(robots, robot_id, robot_j)

def evade_crash_back2(robots, robot_i, robot_j):
    r_action[robot_j][0] = -2
    r_action[robot_j][1] = - np.sign(r_action[robot_j][1]) * np.pi

def evade_crash_simple2(robots, robot_i, robot_j):
    second = 0.1
    v1 = np.sqrt(np.square(robots[robot_i]['x_line_speed']) + np.square(robots[robot_i]['y_line_speed']))
    x1 = robots[robot_i]['x'] + v1 * np.cos(robots[robot_i]['direction']) * second
    y1 = robots[robot_i]['y'] + v1 * np.sin(robots[robot_i]['direction']) * second
...
def evade_crash2(robots, robot_i, robot_j):
    agents = [-1, -1, -1, -1]
    max_speed = 6
    dt = 1/50
    tau = 5
    ...
    for i, agent in enumerate(agents):
        ...
        if new_speed != []: 
            ...
            delta_direction = robot_direction - new_speed_direction
            real_delta_direction = compute_real_direction2(delta_direction, new_speed[1])
...
def get_material_locked2(workstations):
    '''获取所有工作站的信息,获取被死锁了的材料。(即该材料类被没有工作站能接收去放)
    '''
    # 材料有1-7个类别。设立空的set集合，遍历station，找到所有空缺的材料类别添加到set里面。
    # set集合中没有1-7的哪个类别，哪个类别就被锁住了。
...
def r_next_pass2(r_next, robots, workstations, robot_i, station_i, flag=0):
    """判断机器人 i 能够去工作台 station_i。买或者卖
    flag=0 表示机器人 i 能否去工作台买
    flag=1 表示机器人 i 能否去工作台卖
    返回是否成功进行买卖
...
def handle_module2(workstations, robots, frame_id, money):
    global r_action
    global locked_robots
    r_action = [[-1] * 5, [-1] * 5, [-1] * 5, [-1] * 5]
    locked_robots = []
    # Maintain the distance for each robot and s_type for stations.
    r_distance, r_priority, sell_priority = maintain_varible2(workstations, robots)
    
    # Update r_next for each robot.
    find_target2(r_distance, r_priority, sell_priority, workstations, robots)
    
    move_target2(r_distance, workstations, robots)
    
    # Check and update buy, sell, destroy action for each robot.
    check_action2(workstations, robots)
...
def maintain_varible3(workstations, robots):

    global locked_robots
    # 获取每一帧，物品 1-7, 哪个类别物品锁住了。
    material_locked_list = get_material_locked3(workstations)
    for robot_id in range(len(robots)):
        product = robots[robot_id]["if_product"]
        if product != 0 and product in material_locked_list:
            # 机器人有材料，且没地方放
    ...
    return r_priority, sell_priority


def check_action3(workstations, robots):
    # Check for buy, sell, destroy for each robot.
    for robot_id in range(len(robots)):
        target_station = r_next[robot_id]
        robot_in_station = robots[robot_id]["if_station"]
...
def find_target3(r_priority, sell_priority, workstations, robots):
    alpha = 2
    if frame_id > 8500:
        alpha = 100
        ...
        if r_order[robot_id] == 0:  # 如果当前机器人没有靶订单，则设置靶订单
            ...
            if robots[robot_id]['if_product'] == 0:
                ...
                for i in range(len(r_priority)):
                    ...
                    if prior > max_priority and r_next_pass3(r_next, robots, workstations,
                                                            robot_id, i, flag=0):
                        max_priority = prior
                        r_next[robot_id] = i
                        r_order[robot_id] = 1
                ...
                if product_id == 1:
                    ...
                    max_priority = -np.inf
                    for i in range(len(sell_priority_copy)):
                        prior = sell_priority_copy[i] / r_s_dis[robot_id][i]
                        if prior > max_priority and product_id in find_materials_id(
                                workstations[i]['type'], workstations[i]['m_state']) and r_next_pass3(r_next, robots,
                                                                                                     workstations,
                                                                                                     robot_id, i,
                                                                                                     flag=1):
                            max_priority = prior
            ...
            if robots[robot_id]['if_product'] == 0:
                ...
                product_type = robots[robot_id]["if_product"]
                if product_type not in find_materials_id(workstations[station_id]["type"],
                                                         workstations[station_id]["m_state"]):
                    r_order[robot_id] = 0


def compute_real_direction3(delta_direction, delta_y):
    # 区分象限，以机器人的坐标为 (0,0)
    if delta_y > 0:
        if delta_direction > 0 and delta_direction < np.pi:
            real_delta_direction = - delta_direction
...
def move_target3(workstations, robots):
    global locked_robots
    global r_next
    global r_action
    global r_order
    ...
    for robot_id in range(len(robots)):
        ...
        delta_direction = robot_direction - direction
        real_delta_direction = compute_real_direction3(delta_direction, delta_y)
        ...
        eps = 2
        theta = 0.1
        if 1 in dis_wall3(robot_id, eps):
            if robot_direction > (np.pi / 2) + theta and robot_direction < 3 * np.pi / 2 - theta:
                r_action[robot_id][0] = 2
        if 2 in dis_wall3(robot_id, eps):
            if robot_direction > np.pi + theta and robot_direction < 2 * np.pi - theta:
                r_action[robot_id][0] = 2
        if 3 in dis_wall3(robot_id, eps):
            if (robot_direction >= 0 and robot_direction < np.pi / 2 - theta) or (
                    robot_direction > 3 * np.pi / 2 + theta and robot_direction < 2 * np.pi):
                r_action[robot_id][0] = 2
        if 4 in dis_wall3(robot_id, eps):
            if robot_direction > 0 + theta and robot_direction < np.pi - theta:
                r_action[robot_id][0] = 2
            ...
            if robot_j > robot_id:
                if if_crash(robots, robot_id, robot_j):
                    if 0:
                        ...
                        evade_crash_simple3(robots, robot_id, robot_j)
                    else:
                        evade_crash3(robots)


def evade_crash_simple3(robots, robot_i, robot_j):
    second = 0.1
    v1 = np.sqrt(np.square(robots[robot_i]['x_line_speed']) + np.square(robots[robot_i]['y_line_speed']))
    x1 = robots[robot_i]['x'] + v1 * np.cos(robots[robot_i]['direction']) * second
    y1 = robots[robot_i]['y'] + v1 * np.sin(robots[robot_i]['direction']) * second
...
def evade_crash3(robots):
    ...
    for robot_id in range(len(robots)):
        ...
        delta_y = workstations[station_target]["y"] - robots[robot_id]['y']
        dis = np.sqrt(np.square(delta_x) + np.square(delta_y))
        agents[robot_id] = Agent([robots[robot_id]["x"], robots[robot_id]["y"]],
                                 [robots[robot_id]["x_line_speed"], robots[robot_id]["y_line_speed"]], \
                                 0.53 if robots[robot_id]["if_product"] else 0.45, max_speed,
                                 [max_speed * delta_x / dis, max_speed * delta_y / dis])
    ...
    for i, agent in enumerate(agents):
        ...
        if new_speed != []:
            ...
            delta_direction = robot_direction - new_speed_direction
            real_delta_direction = compute_real_direction3(delta_direction, new_speed[1])
...
def get_material_locked3(workstations):
    '''获取所有工作站的信息,获取被死锁了的材料。(即该材料类被没有工作站能接收去放)
    '''
    # 材料有1-7个类别。设立空的set集合，遍历station，找到所有空缺的材料类别添加到set里面。
    # set集合中没有1-7的哪个类别，哪个类别就被锁住了。
...
def r_next_pass3(r_next, robots, workstations, robot_i, station_i, flag=0):
    """判断机器人 i 能够去工作台 station_i。买或者卖
    flag=0 表示机器人 i 能否去工作台买
    flag=1 表示机器人 i 能否去工作台卖
    返回是否成功进行买卖
...
def handle_module3(workstations, robots, frame_id, money):
    global r_action
    global locked_robots
    r_action = [[-1] * 5, [-1] * 5, [-1] * 5, [-1] * 5]
    locked_robots = []
    # Maintain the distance for each robot and s_type for stations.
    r_priority, sell_priority = maintain_varible3(workstations, robots)

    # Update r_next for each robot.
    find_target3(r_priority, sell_priority, workstations, robots)
    move_target3(workstations, robots)

    # Check and update buy, sell, destroy action for each robot.
    check_action3(workstations, robots)
...
def maintain_varible4(workstations, robots):
    # Compute distance between robot and station
    for robot_id in range(len(robots)):
        robot_i = []
        for station_id in range(len(workstations)):
    ...
    global locked_robots
    # 获取每一帧，物品 1-7, 哪个类别物品锁住了。
    material_locked_list = get_material_locked4(workstations)
    for robot_id in range(len(robots)):
        product = robots[robot_id]["if_product"]
        if product != 0 and product in material_locked_list:
            # 机器人有材料，且没地方放
    ...
    for station_id in range(len(workstations)):
        ...
        if workstations[station_id]['type'] in [1,2]:
            priority = 20

        if workstations[station_id]['type'] in [4]:
            priority = 30

        if workstations[station_id]['type'] in [5, 6]:
            priority = 20
        ...
        if frame_id <= 50:
            if workstations[station_id]['type'] in [3]:
                priority = -np.inf
            elif workstations[station_id]['type'] in [1,2]:
                priority = 20
            else:
                priority = -9999
        r_priority.append(priority)
...
def check_action4(workstations, robots):
    # Check for buy, sell, destroy for each robot.
    for robot_id in range(len(robots)):
        target_station = r_next[robot_id]
        robot_in_station = robots[robot_id]["if_station"]
...
def find_target4(r_distance, r_priority, sell_priority, workstations, robots):
    alphamai = 1.1
    alpha = 50
    r_s_dis = r_s_distance(robots, workstations)
    r_s_dis_mai = r_s_distance_mai(robots, workstations)
    for robot_id in range(len(robots)):
        if r_order[robot_id] == 0:  # 如果当前机器人没有靶订单，则设置靶订单
            ...
            if robots[robot_id]['if_product'] == 0:
                ...
                # log.write_string(f"prior: {r_priority}")
                for i in range(len(r_priority)):
                    prior = r_priority[i] - alphamai * r_s_dis_mai[robot_id][i]
                    if prior > max_priority and r_next_pass4(r_next, robots, workstations,
                                                            robot_id, i, flag=0):
                        max_priority = prior
                        r_next[robot_id] = i
                        r_order[robot_id] = 1
                ...
                if product_id == 1:
                    ...
                    for i in range(len(sell_priority_copy)):
                        prior = sell_priority_copy[i] * alpha * r_s_dis[robot_id][i]
                        if prior > max_priority and product_id in find_materials_id(
                                workstations[i]['type'], workstations[i]['m_state']) and r_next_pass4(r_next, robots,
                                                                                                     workstations,
                                                                                                     robot_id, i,
                                                                                                     flag=1):
                            max_priority = prior
...
def compute_real_direction4(delta_direction, delta_y):
    # 区分象限，以机器人的坐标为 (0,0)
    if delta_y > 0:
        if delta_direction > 0 and delta_direction < np.pi:
            real_delta_direction = - delta_direction
...
def move_target4(r_distance, workstations, robots):
    global locked_robots
    global r_next
    global r_action
    global r_order
    ...
    for robot_id in range(len(robots)):
        ...
        delta_direction = robot_direction - direction
        real_delta_direction = compute_real_direction4(delta_direction, delta_y)
        ...
        if delta_dis > eps:
            ...
            if abs(real_delta_direction) > 0.2:
                # r_action[robot_id][0] = delta_dis * (6 / eps)
                r_action[robot_id][0] = 0
                r_action[robot_id][1] = np.pi if real_delta_direction >= 0 else -np.pi
            else:
        ...
        eps = 2
        theta = 0.1
        if 3 in dis_wall4(robot_id, eps):
            if robot_direction > np.pi + theta and robot_direction < 2 * np.pi - theta:
                if abs(real_delta_direction) > 0.7:
                    # r_action[robot_id][0] = delta_dis * (6 / eps)
                    r_action[robot_id][0] = 0
            ...
            if robot_j > robot_id:
                if if_crash(robots, robot_id, robot_j):
                    if frame_id % 3 == 0 or frame_id % 3 == 1:
                        evade_crash4(robots, robot_id, robot_j)
                    elif frame_id % 3 == 2:
                        evade_crash_simple4(robots, robot_id, robot_j)
                    # evade_crash(robots, robot_id, robot_j)
                    # evade_crash_simple(robots, robot_id, robot_j)
                    

def evade_crash_simple4(robots, robot_i, robot_j):
    second = 0.1
    v1 = np.sqrt(np.square(robots[robot_i]['x_line_speed']) + np.square(robots[robot_i]['y_line_speed']))
    x1 = robots[robot_i]['x'] + v1 * np.cos(robots[robot_i]['direction']) * second
    y1 = robots[robot_i]['y'] + v1 * np.sin(robots[robot_i]['direction']) * second
...
def evade_crash4(robots, robot_i, robot_j):
    agents = [-1, -1, -1, -1]
    max_speed = 6
    dt = 1/50
    tau = 5
    ...
    for i, agent in enumerate(agents):
        ...
        if new_speed != []: 
            ...
            delta_direction = robot_direction - new_speed_direction
            real_delta_direction = compute_real_direction4(delta_direction, new_speed[1])
...
def get_material_locked4(workstations):
    '''获取所有工作站的信息,获取被死锁了的材料。(即该材料类被没有工作站能接收去放)
    '''
    # 材料有1-7个类别。设立空的set集合，遍历station，找到所有空缺的材料类别添加到set里面。
    # set集合中没有1-7的哪个类别，哪个类别就被锁住了。
...
def r_next_pass4(r_next, robots, workstations, robot_i, station_i, flag=0):
    """判断机器人 i 能够去工作台 station_i。买或者卖
    flag=0 表示机器人 i 能否去工作台买
    flag=1 表示机器人 i 能否去工作台卖
    返回是否成功进行买卖
...
def handle_module4(workstations, robots, frame_id, money):
    global r_action
    global locked_robots
    r_action = [[-1] * 5, [-1] * 5, [-1] * 5, [-1] * 5]
    locked_robots = []
    # Maintain the distance for each robot and s_type for stations.
    r_distance, r_priority, sell_priority = maintain_varible4(workstations, robots)

    # Update r_next for each robot.
    find_target4(r_distance, r_priority, sell_priority, workstations, robots)
    move_target4(r_distance, workstations, robots)

    # Check and update buy, sell, destroy action for each robot.
    check_action4(workstations, robots)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        read_util_ok()
        if len(workstations) == 43:
             # The core logical handle module
            handle_module1(workstations, robots, frame_id, money)
            # The respond to each frame. Action for each robot.
            respond_module1()
            finish()
        elif len(workstations) == 25:
            # The core logical handle module
            handle_module2(workstations, robots, frame_id, money)
            # The respond to each frame. Action for each robot.
            respond_module2()
            finish()
        elif len(workstations) == 50:
            # The core logical handle module
            handle_module3(workstations, robots, frame_id, money)
            # The respond to each frame. Action for each robot.
            respond_module3()
            finish()
        elif len(workstations) == 18:
            # The core logical handle module
            handle_module4(workstations, robots, frame_id, money)
            # The respond to each frame. Action for each robot.
            respond_module4()
            finish()
